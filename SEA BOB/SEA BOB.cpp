...
void ShowRecord()
{
    ...
    wchar_t rec_txt[100]{ L"НАЙ-ВЕЛИК ИГРАЧ: " };
    wchar_t add[5]{ L"\0" };
    ...
    Draw->BeginDraw();
    Draw->DrawBitmap(bmpIntro[0], D2D1::RectF(0, 0, scr_width, scr_height));
    Draw->DrawTextW(rec_txt, result, bigFormat, D2D1::RectF(50.0f, 100.0f, scr_width, scr_height), hgltBrush);
    Draw->EndDraw();
    Sleep(3500);
}
void SaveGame()
...
void ShowHelp()
{
    ...
    if (statBckgBrush && but1BckgBrush && but2BckgBrush && but3BckgBrush && txtBrush && hgltBrush && txtBrush && nrmFormat)
        Draw->DrawTextW(help_txt, result, midFormat, D2D1::RectF(50.0f, 200.0f, scr_width, scr_height), hgltBrush);
    Draw->EndDraw();
